...
export class SkipRequestSignal extends Error {
  readonly kind = 'skip-request-signal' as const;
...
/** Read-only execution location surface for rq.execution.location. */
export interface ScriptExecutionLocation extends ReadonlyArray<string> {
  readonly current: string | undefined;
}

export interface RqExecutionNamespace {
  setNextRequest(nameOrNull: string | null): void;
  location: ScriptExecutionLocation;
  // skipRequest is added conditionally (pre-request phase only) — see factory.
  // runRequest is added only when the engine supplies a runRequestImpl (ADR-169).
 ...
 * settles.
 */
export function createExecutionNamespace(
  collector: ExecutionDirectiveCollector,
  phase: ScriptPhase,
  location: readonly string[],
  runRequestImpl?: RunRequestImpl,
): RqExecutionNamespace & { skipRequest?: () => never } {
  ...
  const locationArr: ScriptExecutionLocation = Object.assign([...location], { current });
  Object.freeze(locationArr);

  const ns: RqExecutionNamespace = {
    setNextRequest(nameOrNull: string | null): void {
