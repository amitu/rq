import type { EnvironmentVariables, VariableData } from '../model.js';
import type { MutationDiff, MutationVariables } from './host-types.js';
import type { ScriptExecutionContext } from '../execution.js';

import type { RawMutationEntry, RawScopeMutations } from '../definitions/rqMethods.js';
 ...
 */
export function inflateMutations(rawMutations: RawScopeMutations, context: ScriptExecutionContext): MutationDiff {
...
function inflateScope(
  rawEntries: Record<string, RawMutationEntry | null>,
  contextVars: EnvironmentVariables,
  isRuntimeScope: boolean,
): MutationVariables {
  const result: MutationVariables = {};
  ...
  for (const [key, entry] of Object.entries(rawEntries)) {
    ...
    if (existing) {
      ...
      const newVar: VariableData = createDefaultVariableData(entry.value, entry.type);
      if (isRuntimeScope) {
