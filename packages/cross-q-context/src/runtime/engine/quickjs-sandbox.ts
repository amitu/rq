...
import {
  REQUIRE_ISOLATE_SHIM,
  extractRequireIds,
  prebundleRequires,
  resolveRequire,
} from './isolated/isolated-require.js';
import { createSourceBundler } from './isolated/source-bundler.js';

import type { QuickJsHostConfig } from './engine.js';
import type { BundleRequireDeps } from './isolated/isolated-require.js';
...
export const NODE_QUICKJS_HOST: QuickJsHostConfig = {
  createModule: () =>
  ...
  createRequireSupport: ({ resolver, bundleCache }) => ({
    ...
    prepare: async (input) => {
      ...
      const requireDeps: BundleRequireDeps = {
        bundler,
        userPackages: input.userPackages,
        blacklistedPackages: input.blacklistedPackages ?? [],
        vendorIife: (id: string): string | undefined => vendorIifeLookup[id],
      ...
      return { resolve: (id: string) => resolveRequire(id, requireDeps, prebuilt) };
    },
  }),
