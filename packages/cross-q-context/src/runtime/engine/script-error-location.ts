 ...
 */
export function scriptFilenameForPhase(phase: ScriptPhase): string {
...
export function parseScriptErrorLocation(
  ...
  for (const rawLine of rawStack.split('\n')) {
    ...
    if (match === null) {
      ...
      if (!FRAME_LINE_RE.test(rawLine) && rawLine.trim() !== '') displayLines.push(rawLine);
      continue;
    }
    const lineStr = match[1];
    const columnStr = match[2];
    if (lineStr === undefined || columnStr === undefined) continue;
    ...
    if (line < 1 || line > scriptLineCount) continue;
    if (primary === undefined) primary = { line, column };
