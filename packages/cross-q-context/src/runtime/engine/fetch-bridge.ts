...
import type { BodyEncoding, SendRequestHost, SerializedFetchRequest, SerializedFetchResponse } from './host-types.js';
import type { SsrfPolicy } from './ssrf-guard.js';

/**
 * Local `type` mirror of `SerializedFetchRequest` (the copy-in shape). A `type`
 ...
 * the shared-types `SerializedFetchRequest` interface (no cast).
 */
...
/**
 * Local `type` mirror of `SerializedFetchResponse` (the copy-out shape). Carries
 * `bodyEncoding` so a base64 body round-trips losslessly through the flat `body`
 * string (ADR-153). Assignable both ways with the shared-types interface (no cast).
 */
 ...
 */
async function directFetch(req: FetchRequestData, guardedFetch: typeof fetch): Promise<FetchResponseData> {
 ...
 */
export function createFetchBridge(host?: SendRequestHost, policy: SsrfPolicy = CLIENT_SSRF_POLICY): SafeBridge {
  ...
  const handler = async (req: FetchRequestData): Promise<FetchResponseData> => {
    if (host === undefined) {
    ...
    const request: SerializedFetchRequest = req;
    ...
    const response: SerializedFetchResponse = envelope.response;
