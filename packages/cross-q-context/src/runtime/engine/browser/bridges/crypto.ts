...
const HASHERS: Record<HashAlgo, CHash> = { sha1, sha256, sha384, sha512, md5 };
...
export type CryptoResult = { readonly digest: string } | { readonly bytes: Uint8Array } | { readonly uuid: string };
...
function encode(bytes: Uint8Array, outputEncoding: 'hex' | 'base64'): string {
 ...
 */
export function browserCryptoHandler(req: CryptoOp): CryptoResult {
  switch (req.op) {
    case 'hash': {
