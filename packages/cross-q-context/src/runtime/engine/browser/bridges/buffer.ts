...
export type BufferResult = { readonly bytes: Uint8Array } | { readonly text: string };
...
for (let i = 0; i < BASE64_ALPHABET.length; i++) BASE64_LOOKUP.set(BASE64_ALPHABET[i] ?? '', i);
...
function base64ToBytes(input: string): Uint8Array {
  ...
  for (const char of input) {
    ...
    if (value !== undefined) symbols.push(value);
  }
...
function hexToBytes(input: string): Uint8Array {
  ...
  for (let i = 0; i + 1 < input.length; i += 2) {
    ...
    if (Number.isNaN(byte) || !/^[0-9a-fA-F]{2}$/.test(input.slice(i, i + 2))) break;
    out[count++] = byte;
  }
...
function toSingleByte(input: string): Uint8Array {
  const out = new Uint8Array(input.length);
  for (let i = 0; i < input.length; i++) out[i] = input.charCodeAt(i) & 0xff;
  return out;
}

function fromSingleByte(bytes: Uint8Array, maskHighBit: boolean): string {
  let out = '';
  for (const byte of bytes) out += String.fromCharCode(maskHighBit ? byte & 0x7f : byte);
  return out;
}

function encodeToBytes(input: string, encoding: BufferEncodingSubset): Uint8Array {
  switch (encoding) {
    case 'utf8':
    case 'utf-8':
...
function decodeToText(bytes: Uint8Array, encoding: BufferEncodingSubset): string {
  switch (encoding) {
    case 'utf8':
    case 'utf-8':
...
export function browserBufferHandler(req: BufferOp): BufferResult {
