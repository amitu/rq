 ...
 * them. This is not duplication for its own sake: `createSafeBridge`'s `Copyable`
 * constraint is `{ readonly [key: string]: Copyable }`, and a TypeScript
 * `interface` has no implicit index signature — so the shared-types *interfaces*
 * are not assignable to it, while these structurally-identical `type` aliases are.
 * Both mirrors remain assignable in both directions with the shared-types
 * interfaces, so no cast is needed anywhere (`gr-no-unsafe-cast`).
 */
...
export function createBrowserFetchHandler(
  host: SendRequestHost,
): (req: FetchRequestData) => Promise<FetchResponseData> {
  return async (req: FetchRequestData): Promise<FetchResponseData> => {
...
export function createBrowserFetchBridge(host: SendRequestHost): SafeBridge {
