...
export const STRICT_SSRF_POLICY: SsrfPolicy = { allowPrivateNetwork: false };

/** Lenient posture for client hosts — preserves `localhost`/LAN access. */
export const CLIENT_SSRF_POLICY: SsrfPolicy = { allowPrivateNetwork: true };

/** Hostnames that must never be reachable regardless of resolved IP or policy. */
const BLOCKED_HOSTNAMES: ReadonlySet<string> = new Set(['metadata.google.internal', 'metadata.goog', 'metadata']);
...
function isIpv4LinkLocal(value: number): boolean {
  return inRange(value, '169.254.0.0', 16);
}

/** Loopback / private / reserved ranges — blocked only under the strict policy. */
function isIpv4Private(value: number): boolean {
  return (
    inRange(value, '0.0.0.0', 8) || // "this host"
...
function parseIpv6(input: string): readonly number[] | null {
  ...
  if (zone !== -1) str = str.slice(0, zone); // drop scope id (fe80::1%eth0)
  ...
  if (gap < 1) return null; // `::` must stand for ≥1 zero group
  return [...head, ...new Array<number>(gap).fill(0), ...tail];
}

function hextetsMatch(h: readonly number[], prefix: readonly number[]): boolean {
 ...
 */
function isIpv6HextetsBlocked(h: readonly number[], allowPrivateNetwork: boolean): boolean {
 ...
 */
export async function assertUrlAllowed(rawUrl: string, policy: SsrfPolicy): Promise<void> {
  ...
  if (isIP(hostname) !== 0) {
    if (isAddressBlocked(hostname, policy)) throw new SsrfBlockedError('blocked-address');
    return;
  }

  // Resolve and validate EVERY address the hostname maps to.
  const resolved = await lookup(hostname, { all: true, verbatim: true });
  if (resolved.length === 0) throw new SsrfBlockedError('blocked-address');
  for (const { address } of resolved) {
    if (isAddressBlocked(address, policy)) throw new SsrfBlockedError('blocked-address');
  }
}
 ...
 */
export function createGuardedLookup(policy: SsrfPolicy): LookupFunction {
...
function blockedLookupError(): NodeJS.ErrnoException {
  const err: NodeJS.ErrnoException = new SsrfBlockedError('blocked-address');
  err.code = 'EACCES';
  return err;
}
 ...
 */
export function createGuardedFetch(baseFetch: typeof fetch, policy: SsrfPolicy): typeof fetch {
  const guarded = async (input: Parameters<typeof fetch>[0], init?: Parameters<typeof fetch>[1]): Promise<Response> => {
