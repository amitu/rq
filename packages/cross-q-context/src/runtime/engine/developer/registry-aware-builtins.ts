 ...
 */
function wrapCallbackLast<T extends object>(mod: T, registry: AsyncRegistry<never>): T {
  const wrapped = new Map<string, unknown>();

  return new Proxy(mod, {
    get(target, prop, receiver): unknown {
      const value: unknown = Reflect.get(target, prop, receiver);
      if (!isFunction(value) || typeof prop !== 'string') return value;
      ...
      const wrapper = (...args: unknown[]): unknown => {
        ...
        if (!isFunction(last)) return Reflect.apply(value, target, args);

        const settle = registry.register();
        let settled = false;
        const trackedCallback = (...callbackArgs: unknown[]): unknown => {
          if (!settled) {
            settled = true;
            settle();
          }
 ...
 * (`timers.js:126-133`).
 */
function registryTimersModule(timers: RegistryTimerGlobals): Record<string, unknown> {
  const setTimeoutFn = timers.setTimeout;
  return {
    ...timers,
    setImmediate: isFunction(setTimeoutFn)
      ? (fn: unknown, ...args: unknown[]): unknown => setTimeoutFn(fn, 0, ...args)
      : setTimeoutFn,
 ...
 */
export function applyDeveloperAsyncTreatment(
  mod: unknown,
  treatment: DeveloperAsyncTreatment,
  registry: AsyncRegistry<never>,
  timers: RegistryTimerGlobals,
): unknown {
  switch (treatment) {
