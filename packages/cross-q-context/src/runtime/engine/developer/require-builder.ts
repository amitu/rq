...
import type { DeveloperAsyncTreatment, PackageResolver } from '../../index.js';
...
const DEVELOPER_ASYNC_BY_ID = new Map<string, DeveloperAsyncTreatment>(
  NODE_BUILTIN_PACKAGES.flatMap((p) => [
    [p.id, p.developerAsync] as const,
    [`node:${p.id}`, p.developerAsync] as const,
  ]),
);
...
const safeNodeBuiltinIds: ReadonlySet<string> = new Set(NODE_BUILTIN_PACKAGES.map((p) => p.id));
const BLOCKED_NODE_BUILTINS = new Set(
  builtinModules.filter((m) => !m.startsWith('_') && !safeNodeBuiltinIds.has(m)).flatMap((m) => [m, `node:${m}`]),
);

// Pre-compute lookup map: package id → { iife, globalName }
const packageMap = new Map<string, { iife: string; globalName: string }>();
for (const pkg of EXTERNAL_BUILTIN_PACKAGES) {
  const iife: string | undefined = VENDOR_IIFES[pkg.id];
  if (iife) {
...
for (const [id, pkg] of packageMap) {
  ...
  try {
    ...
    (0, eval)(iifeCode);
    const mod = (globalThis as Record<string, unknown>)[pkg.globalName];
    if (mod !== undefined) {
...
function mapResolverError(id: string, err: unknown): Error {
  const message = err instanceof Error ? err.message : String(err);
  const missingMatch = /Cannot find module '(@[^'/]+\/[^'/]+|[^'/]+)'/.exec(message);
  if (missingMatch) {
    const missingDep = missingMatch[1];
    return createPackageError(
      `Package '${id}' failed to load: missing dependency '${missingDep}'. Add '${missingDep}' to your script imports.`,
      { cause: err },
    );
  }
 ...
 */
export function createRequireFn(
  userPackages?: Record<string, string>,
  vmEvaluator?: VmPackageEvaluator,
  resolver?: PackageResolver,
  contextId?: string,
  blacklistedPackages: string[] = [],
  asyncTreatment?: { readonly registry: AsyncRegistry<never>; readonly timers: RegistryTimerGlobals },
): (id: string) => unknown {
  ...
  return function require(id: string): unknown {
    ...
    if (ALLOWED_NODE_BUILTINS.has(id)) {
      const builtinMod: unknown = nodeRequire(id);
      ...
      const treated =
        asyncTreatment === undefined
          ? builtinMod
          : applyDeveloperAsyncTreatment(
              builtinMod,
              DEVELOPER_ASYNC_BY_ID.get(id) ?? 'not-an-async-source',
              asyncTreatment.registry,
              asyncTreatment.timers,
            );
