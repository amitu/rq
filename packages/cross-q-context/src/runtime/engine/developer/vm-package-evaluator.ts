 ...
 */
function adjustLineNumber(message: string): string {
  return message.replace(/\bline (\d+)\b/g, (_match, lineStr: string) => {
 ...
 */
function formatImportChain(evaluationStack: readonly string[]): string {
  if (evaluationStack.length === 0) return '';
  if (evaluationStack.length === 1) return ` (imported by '${evaluationStack[0]}')`;
 ...
 */
export function createVmEvaluator(vmContext: vm.Context): VmPackageEvaluator {
  const evaluationStack: string[] = [];

  return function evaluate(name: string, source: string): unknown {
    ...
    try {
      ...
      const prevModule = (vmContext as Record<string, unknown>)['module'];
      (vmContext as Record<string, unknown>)['module'] = mod;
      ...
      try {
        ...
        (vmContext as Record<string, unknown>)['module'] = prevModule;
      }
