 ...
 * resolution).
 */
let cachedSafeEngine: Sandbox | undefined;

async function resolveSafeEngine(
...
export class DispatchingSandbox implements Sandbox {
  ...
  constructor(
    private readonly developerEngine: NodeSandbox,
    private readonly resolver?: SafePackageResolver,
  ...
  execute(
    ...
    switch (input.mode) {
      ...
      default:
        return resolveSafeEngine(this.resolver, this.bundleCache).then((engine) =>
          engine.execute(input, hostCallbacks),
        );
    }
  }
}
