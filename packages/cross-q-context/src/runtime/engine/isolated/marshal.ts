 ...
 */
export function dumpHandle(ctx: QuickJSAsyncContext, handle: QuickJSHandle): unknown {
  ...
  if (ctx.typeof(handle) === 'object') {
    ...
    const dumpedShallow: unknown = ctx.dump(handle);
    if (dumpedShallow === null) return null;
...
function isArrayBufferHandle(ctx: QuickJSAsyncContext, handle: QuickJSHandle): boolean {
  if (ctx.typeof(handle) !== 'object') return false;
  const ctorHandle = ctx.getProp(handle, 'constructor');
  try {
    if (ctx.typeof(ctorHandle) === 'undefined') return false;
 ...
 */
export function marshalToHandle(ctx: QuickJSAsyncContext, value: unknown): QuickJSHandle {
