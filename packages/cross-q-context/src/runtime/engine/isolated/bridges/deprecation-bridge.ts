 ...
 */
export const WARN_ONLY_IDENTIFIERS: readonly string[] = Object.keys(DEPRECATED_IDENTIFIERS).filter(
  (id) => !(SHIMMED_IDENTIFIERS as readonly string[]).includes(id),
);

/** The four real `rq.*`-delegating shims, re-exported as a plain string[] for the shim/test. */
export const DEPRECATION_SHIMMED_IDENTIFIERS: readonly string[] = SHIMMED_IDENTIFIERS;
 ...
 */
export function createDeprecationBridge(emit: DeprecationEmit): SafeBridge {
  return createIgnoredBridge('__rq_deprecation', (identifier: string, shimmed: boolean): void => {
