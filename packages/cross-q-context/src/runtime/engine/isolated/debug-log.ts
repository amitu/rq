...
function hostStderrWrite(): ((chunk: string) => unknown) | undefined {
  const proc: unknown = Reflect.get(globalThis, 'process');
  if (typeof proc !== 'object' || proc === null) return undefined;
  const stderr: unknown = Reflect.get(proc, 'stderr');
  if (typeof stderr !== 'object' || stderr === null) return undefined;
  const write: unknown = Reflect.get(stderr, 'write');
  return typeof write === 'function' ? (chunk: string): unknown => Reflect.apply(write, stderr, [chunk]) : undefined;
}
...
export function dlog(area: string, message: string, extra?: Record<string, unknown>): void {
  if (!ENABLED) return;
  try {
    const now = Date.now();
