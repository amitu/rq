 ...
 */
export const DEPRECATED_IDENTIFIERS = {
 ...
 */
export function formatDeprecationMessage(identifier, policy) {
    switch (policy.kind) {
 ...
 */
export function createDeprecationProxy(identifier, emit) {
    let warned = false;
    const fire = () => {
        if (warned)
            return;
    ...
    const target = Object.assign(function deprecated() {
        return proxy;
    }, undefinedIndex());
    const handler = {
        get(_t, prop) {
 ...
 */
export const SHIMMED_IDENTIFIERS = ['globals', 'environment', 'responseBody', 'responseCode'];
/**
...
function isResponseView(value) {
    if (typeof value !== 'object' || value === null)
        return false;
    if (!('code' in value) || !('text' in value))
        return false;
    const record = value;
 ...
 */
function asVariableNamespace(delegate) {
 ...
 */
function createNamespaceShim(identifier, target, emit) {
    let warned = false;
    const fire = () => {
        if (warned)
            return;
    ...
    const delegate = typeof target === 'object' && target !== null ? target : {};
 ...
 */
export function createDeprecatedPostmanShims(rq, emit) {
