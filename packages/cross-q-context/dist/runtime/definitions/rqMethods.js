...
// ---------------------------------------------------------------------------
function createVariableScope(contextVars, rawMutations, scopeKey, options) {
    return {
        ...
        get(key) {
            ...
            if (pending === null)
                return undefined;
            if (pending)
                return coerceValueByType(pending.value, pending.type);
 ...
 */
export function createRqNamespace(executionState, libs, context, eventName, cookieBridge, entryType, fetchImpl = globalThis.fetch, 
...
runRequestImpl) {
     ...
     */
    test.skip = function skip(name, testFn) {
    ...
    const collectionVariables = createVariableScope(context.collectionVariables, executionState.rawMutations, 'collection', { readonly: isCollectionReadonly });
    ...
    if (executionState.requestMutations === undefined)
        executionState.requestMutations = [];
    const requestMutationCollector = { headers: executionState.requestMutations };
    const request = buildScriptRequest(context.request, resolvedEntryType, context.auth ? { auth: context.auth } : undefined, requestMutationCollector);
    /**
    ...
    const assertVaultAccessEnabled = () => {
        if (context.secretsAccessDisabled === true) {
            throw new Error('Vault access from scripts is disabled on this device. Turn on "Allow scripts to access vault secrets" in Settings → Vault to read vault secrets in scripts.');
        }
    };
    const vault = {
        // Returns `any` for parity with the variable-scope getters, sparing scripts from coercing
