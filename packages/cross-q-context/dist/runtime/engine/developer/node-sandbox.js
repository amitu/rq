...
import { buildBatchResult, createBatchOutcome, ON_MESSAGE_TIMEOUT_ERROR, stampMessageIndex, } from '../index.js';
...
import { buildScriptMessage, createDeprecatedPostmanShims, createSendRequest, DEPRECATED_IDENTIFIERS, formatDeprecationMessage, SkipRequestSignal, } from '../../index.js';
...
function normalizeTestResults(results) {
    const parsed = normalizeFromVm(results);
 ...
 */
function isCookieJarMutation(value) {
 ...
 */
function normalizeCookieMutations(mutations) {
    const parsed = normalizeFromVm(mutations);
 ...
 */
function wrapScopeGetsForVmRealm(rq, vmContext) {
    const reviveInRealm = Reflect.get(vmContext, '__rq_reviveArrayInRealm');
    if (typeof reviveInRealm !== 'function')
        return;
    ...
    // in a node:vm realm.
    const revive = (json) => {
        const out = reviveInRealm(json);
        return Array.isArray(out) ? out : [];
    };
    for (const scopeKey of ['environment', 'globals', 'collectionVariables', 'variables']) {
        const scope = rq[scopeKey];
        if (scope === null || typeof scope !== 'object')
            continue;
        const originalGet = Reflect.get(scope, 'get');
        if (typeof originalGet !== 'function')
            continue;
        const boundGet = (key) => Reflect.apply(originalGet, scope, [key]);
        Reflect.set(scope, 'get', (key) => {
...
export class NodeSandbox {
    ...
    async runScript(input, handle, hostCallbacks) {
        ...
        try {
            ...
            const baseFetch = hostCallbacks?.sendRequest !== undefined ? toDelegatedFetch(hostCallbacks.sendRequest) : this.guardedFetch;
            const trackedFetch = (...args) => asyncRegistry.registerPromise(baseFetch(...args));
            ...
            const asyncGlobals = {
                fetch: trackedFetch,
                setTimeout: (fn, ms, ...args) => asyncRegistry.setTimer(() => fn(...args), ms ?? 0),
                ...
                setInterval: (fn, ms, ...args) => {
                    ...
                    const tick = () => {
                        const armed = asyncRegistry.setTimer(() => {
                            ...
                            if (!activeIntervals.has(publicId))
                                return;
                            fn(...args);
            ...
            vmContext['require'] = createRequireFn(input.userPackages, vmEvaluator, this.resolver, contextId, input.blacklistedPackages, 
            ...
            { registry: asyncRegistry, timers: asyncGlobals });
            ...
            const chaiModule = vmContext['__chai'];
            ...
            const lodashModule = vmContext['__lodash'];
            const ajvModule = vmContext['__ajv'];
            ...
            const handlebarsModule = vmContext['__handlebars'];
            ...
            const trackedSendRequest = (requestInput, callback) => asyncRegistry.registerPromise(callback === undefined ? sendRequestImpl(requestInput) : sendRequestImpl(requestInput, callback));
            built.rq['sendRequest'] = trackedSendRequest;
            vmContext['rq'] = built.rq;
            ...
            if (input.messageBatch !== undefined) {
                ...
                handle.push({
                    type: 'result',
                    result: buildBatchResult(outcome, outcome.mutations ? inflateMutations(outcome.mutations, input.context) : {}, normalizeCookieMutations(drainCookieMutations())),
                });
                handle.end();
                return;
            }
            ...
            for (;;) {
                ...
                if (asyncRegistry.holdingCount() === 0)
                    break;
            ...
            if (err !== null && typeof err === 'object') {
                ...
                if (typeof stashed === 'string' && stashed !== '')
                    capturedStack = stashed;
            }
            const rawStack = capturedStack ?? (err instanceof Error ? err.stack : undefined);
            const errorLocation = parseScriptErrorLocation(rawStack, countScriptLines(input.script), scriptFilenameForPhase(input.phase), message);
    ...
    async runMessageBatch(deps) {
        ...
        for (const message of batch) {
            ...
            if (failure !== undefined) {
                ...
                if (failure.message === ON_MESSAGE_TIMEOUT_ERROR)
                    break;
                continue;
            }
