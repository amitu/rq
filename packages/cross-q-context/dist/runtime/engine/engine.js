 ...
 */
import { ARRAY_METHODS_SHIM, CONVENIENCE_GLOBALS_SHIM, DEPRECATED_IDENTIFIERS, formatDeprecationMessage, } from '../index.js';
...
import { AsyncRegistry, CORE_GLOBALS_SHIM, DEPRECATION_ISOLATE_SHIM, ON_MESSAGE_TIMEOUT_ERROR, RQ_COLLECT_EXPR, RQ_ISOLATE_SHIM, RQ_ITERATION_RESET_EXPR, RUN_REQUEST_ISOLATE_SHIM, SANDBOX_DEFAULT_TIMEOUT_MS, UserScriptError, buildBatchResult, countScriptLines, createBatchOutcome, createConsoleBridge, createDeprecationBridge, createInMemoryCookieJarBridge, createRunRequestBridge, createTimerBridges, dlog, inflateMutations, marshalToHandle, parseScriptErrorLocation, pendingAsyncCalls, scriptFilenameForPhase, stampMessageIndex, } from './index.js';
...
function getQuickJsModule(createModule) {
...
function isRequestHeaderMutation(value) {
...
function isExecutionDirective(value) {
    if (typeof value !== 'object' || value === null)
        return false;
    const v = { ...value };
    if (v['kind'] === 'skip-request')
        return true;
...
function isVisualizerDirective(value) {
    if (typeof value !== 'object' || value === null)
        return false;
    const v = { ...value };
 ...
 */
function parseCollected(collectedJson) {
    let collected = {};
    try {
        const parsed = JSON.parse(collectedJson);
        if (parsed && typeof parsed === 'object') {
            ...
            collected = {
                ...
                mutations: obj['mutations'] && typeof obj['mutations'] === 'object'
 ...
 */
function buildResult(collectedJson, context) {
...
export class QuickJsEngine {
    ...
    host;
    constructor(resolver, bundleCache, host) {
    ...
    async runScript(input, handle, hostCallbacks) {
        ...
        const reportTimerCallbackError = (message) => {
            handle.push({
                type: 'log',
                log: {
        ...
        try {
            ...
            installBridge(createConsoleBridge((log) => handle.push({ type: 'log', log }), () => Date.now()));
            ...
            const emitDeprecation = (identifier, opts) => {
            ...
            installBridge(this.host.createFetchBridge(hostCallbacks?.sendRequest));
            ...
            if (hostCallbacks?.runRequest) {
                installBridge(createRunRequestBridge(hostCallbacks.runRequest));
            }
            ...
            const cookieBridgeHandle = createInMemoryCookieJarBridge(input.context.cookieJarSeed);
            const hostAllowlist = new Set((input.context.hostAllowlist ?? []).map((h) => h.toLowerCase()));
            const cookieFnHandle = ctx.newFunction('__rq_cookies', (argsHandle) => {
            ...
            const timeout = { deadline: Date.now() + timeoutMs, interruptCalls: 0, tripped: false };
            runtime.setInterruptHandler(() => {
                const stop = ++timeout.interruptCalls > INTERRUPT_OP_CEILING || Date.now() > timeout.deadline;
                if (stop)
                    timeout.tripped = true;
                return stop;
            });
            ...
            if (input.messageBatch !== undefined) {
                ...
                handle.push({
                    type: 'result',
                    result: buildBatchResult(outcome, outcome.mutations ? inflateMutations(outcome.mutations, input.context) : {}, cookieBridgeHandle.drainMutations()),
                });
                handle.end();
            ...
            if (evalResult.error) {
                ...
                const errorLocation = parseScriptErrorLocation(runStack, countScriptLines(input.script), scriptFilename, runMessage);
            ...
            if (userError !== '' && result.executionDirective?.kind !== 'skip-request') {
                ...
                const errorLocation = parseScriptErrorLocation(userStack, countScriptLines(input.script), scriptFilename, userError);
     ...
     */
    async pumpToSettlement(ctx, runtime, promise, timeout) {
     ...
     */
    async runMessageBatch(ctx, runtime, input, batch, timeout, timeoutMs) {
        const scriptFilename = scriptFilenameForPhase(input.phase);
     ...
     */
    disposeGuest(context, runtime, installedGlobals) {
    ...
    dumpErrorStack(ctx, errorHandle) {
        const dumped = ctx.dump(errorHandle);
        if (dumped && typeof dumped === 'object' && 'stack' in dumped) {
            const s = Reflect.get(dumped, 'stack');
            if (typeof s === 'string')
                return s;
        }
        return undefined;
