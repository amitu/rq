 ...
 */
const GUIDANCE = {
    native_addon: 'it needs a native (.node) addon, which Safe mode cannot load. Switch this request to Developer mode to use it.',
    live_socket: 'it needs a live network socket, which Safe mode does not expose. Use the built-in fetch, or switch this request to Developer mode.',
    live_fs: 'it needs live filesystem access, which Safe mode does not expose. Switch this request to Developer mode to use it.',
    asymmetric_crypto: 'asymmetric crypto (RS256/ES/PS) is not available in Safe mode. Use HS256, or switch this request to Developer mode.',
 ...
 */
export function createImpossiblePackageError(packageId, reason, options) {
    ...
    const err = options?.cause === undefined
