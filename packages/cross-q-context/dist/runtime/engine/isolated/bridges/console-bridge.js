...
const CONSOLE_LEVELS = [LogLevel.log, LogLevel.warn, LogLevel.error, LogLevel.info];
 ...
 */
export function createConsoleBridge(onLog, now) {
    return createIgnoredBridge('__rq_console', (level, argsJson) => {
        let args;
        try {
        ...
        onLog({ level: levelFor(level), args: args, timestamp: now() });
    });
}
/**
