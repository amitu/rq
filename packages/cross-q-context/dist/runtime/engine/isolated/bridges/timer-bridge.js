 ...
 */
export function createTimerBridges(registry, onGuestCallbackError) {
    /**
     ...
     */
    const live = new Map();
    const arm = (guestId, ms) => new Promise((resolve) => {
        const settle = () => {
            resolve(undefined);
        };
    ...
    return [
        createSafeBridge('__rq_setTimer', (guestId, ms) => arm(guestId, ms), {
            async: true,
        }),
        createSafeBridge('__rq_clearTimer', (guestId) => {
        ...
        createIgnoredBridge('__rq_timerError', (message) => {
