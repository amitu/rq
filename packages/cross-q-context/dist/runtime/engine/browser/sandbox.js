 ...
 */
const vendorGlobalNames = Object.fromEntries(EXTERNAL_BUILTIN_PACKAGES.map((pkg) => [pkg.id, pkg.globalName]));
/**
    ...
    return {
        ...
        prepare: () => Promise.resolve({
            resolve: (id) => {
                ...
                if (bridgeGlobal !== undefined)
                    return { kind: 'bridge', global: bridgeGlobal };
                ...
                throw new Error("Installing npm packages isn't available for scripts in the browser. " +
 ...
 */
export function createBrowserQuickJsHost(sendRequestHost) {
    return {
        createModule: getQuickJsModule,
        createRequireSupport: () => createBrowserRequireSupport(),
        valueBridgeFactories: [
            () => createSafeBridge('__rq_buffer', browserBufferHandler),
            () => createSafeBridge('__rq_crypto', browserCryptoHandler),
            () => createSafeBridge('__rq_util_inspect', browserUtilHandler),
            () => createSafeBridge('__rq_zlib', browserZlibHandler),
        ],
